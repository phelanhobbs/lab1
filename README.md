...
- [x] Make checks to see if any of the *dependencies* of the rule have changed, and only runs the recipe if there are changes.

Before wrapping up this section, we need to do some house keeping.

- [x] Run `git status`. You'll notice that hello.txt is shown as a new file.
...
- [X] Create a new file called `.gitignore`, and add a line with `hello.txt`
Now run `git status`. The build artifact is no longer listed as a new file. You will see the ignore file.

- [X] Commit everything, including the `.gitignore` file, before moving to the next section.

## Dependencies
- [X] Our basic rule is pretty trivial. Let's introduce the first member of our toolchain.
...
- [X] Try to run `make main.i`. What output do you predict if you run it a second time?

- [X] Modify the `main.c` file, adding a comment or changing the return value. What do you predict will happen if you run make again?
...
- [X] Commit everything before moving to the next section.

## Cleaning up
A common task is to remove all the build artifacts to start with a clean build. 
- [X] Let's add a rule to remove the two artifacts we have.

```
clean:
    rm -f main.i hello.txt
```
- [X] Run `make clean`. What do you predict will happen if you run it again?
...
- [x] Let's continue with the compiler and assembler.
...
- [x] Create another file with an empty function in it.
...
- [x] Modify your rule to look like the following.
```
%.o: %.s
    $(AS) second.s -o second.o
```
...
- [x] Update your rules to use patterns and automatic variables.
...
- [x] Leave the recipe empty.
- [x] Remember to add `all` to the `.PHONY` list.
...
## CI/CD
- [x] Create a file `.github/workflows/main.yml`. Add the following configuration.
